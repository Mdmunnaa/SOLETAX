...
from .models import Invoice, Expense, TaxProfile, RecordEditHistory, SubmissionLog
...
class TaxProfileAdmin(admin.ModelAdmin):
    list_display = ('user', 'mtd_signup_status', 'accounting_basis', 'period_type', 'is_hmrc_ready_display')
    list_filter = ('mtd_signup_status', 'accounting_basis', 'period_type')
    search_fields = ('user__username', 'utr', 'business_id')
    readonly_fields = ('created_at', 'updated_at')

    @admin.display(boolean=True, description='HMRC ready')
...
class RecordEditHistoryAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        return False  # audit log — created only by app logic, never manually
